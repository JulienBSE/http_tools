...
if (!existsSync(CHEMIN_BDD)) {
  console.warn(
    `⚠️  Attention : La base de données n'existe pas à ${CHEMIN_BDD}`
  );
}
...
// Créer le dossier parent de la base de données si nécessaire (pour Docker volume)
// Cela permet de créer /app/data si le volume est monté sur /app/data
const dbDir = dirname(dbPath);
if (dbDir && dbDir !== __dirname && !existsSync(dbDir)) {
  mkdirSync(dbDir, { recursive: true });
  console.log(`📁 Dossier BDD créé : ${dbDir}`);
}
